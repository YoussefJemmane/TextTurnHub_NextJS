...
export default function MyExchangeRequestsPage() {
    ...
    const fetchRequests = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/waste-exchanges?type=sent&${params}`
        );
        if (!response.ok) {
  ...
  const handleGenerateReceipt = async (id: number) => {
    try {
      const response = await fetch(`/api/waste-exchanges/${id}/receipt`, {
        method: "GET",
      });
      if (response.ok) {
        ...
        a.href = url;
        a.download = `exchange-receipt-${id}.pdf`;
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-lg text-gray-600">
            Track all your textile waste exchange requests
          </p>
        </div>
      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        {req.status === "accepted" && (
                          <button
                            ...
                            onClick={() => handleGenerateReceipt(req.id)}
                          >
