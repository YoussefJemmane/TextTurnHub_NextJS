...
export default function MyTextileWasteListPage() {
  ...
  const fetchTextileWaste = async () => {
    try {
      ...
      const response = await fetch(
        `/api/textile-waste/my-listings?${queryParams}`
      );
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedItems.length} selected listings?`
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/textile-waste/my-listings?ids=${selectedItems.join(",")}`,
        {
          method: "DELETE",
        }
      );
  ...
  const filteredAndSortedData = textileWaste
    .filter((item) => {
      ...
      const matchesMaterial =
        filterMaterial === "all" ||
        item.material_type.toLowerCase() === filterMaterial.toLowerCase();
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              Please sign in to view your textile waste listings.
            </p>
            <a
              href="/auth/signin"
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            My Textile Waste Listings
          </h1>
          <p className="text-lg text-gray-600">
            Manage your textile waste inventory and listings
          </p>
        </div>
        <a
      ...
      <div className="flex items-center justify-between mb-6">
        <p className="text-gray-600">
          Showing {filteredAndSortedData.length} of {textileWaste.length}{" "}
          listings
        </p>
      ...
      <div className="bg-white/80 backdrop-blur-sm shadow-xl rounded-2xl border border-white/20 overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50/80">
              <tr>
                ...
                <SortableHeader field="price_per_unit">
                  Price/Unit
                </SortableHeader>
        ...
        {filteredAndSortedData.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              No textile waste listings found
            </h3>
            <p className="text-gray-600 mb-4">
